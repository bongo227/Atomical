...
public:
    ...
    inline BasicBlock *current_block() { return _current_block; }

    Value *gen(const Expression *exp);
    void gen_if_branch(const Statement *smt, BasicBlock *parent_block, BasicBlock *end_block);
    BasicBlock *gen_block(BasicBlock *target_block, const std::vector<Statement *> block);
    IrFunction *gen(Function *func);
    Instruction *gen(const Statement *smt);
